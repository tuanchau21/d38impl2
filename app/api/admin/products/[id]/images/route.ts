...
export async function POST(
  ...
  // #region agent log
  debugLog({ location: "app/images/route.ts:entry", message: "POST images entry", data: { id, numId }, hypothesisId: "H2" });
  // #endregion
  try {
    const product = await getProductByIdOrSlug(id);
    // #region agent log
    debugLog({ location: "app/images/route.ts:after getProduct", message: "product lookup done", data: { productFound: !!product, productId: product?.id }, hypothesisId: "H2" });
    // #endregion
    if (!product) {
    ...
    // #region agent log
    debugLog({ location: "app/images/route.ts:after getProductImageCount", message: "image count done", data: { currentCount }, hypothesisId: "H2" });
    // #endregion
    ...
    // #region agent log
    debugLog({ location: "app/images/route.ts:before upload loop", message: "starting upload loop", data: { toUploadLength: toUpload.length }, hypothesisId: "H1" });
    // #endregion
    ...
    // #region agent log
    debugLog({ location: "app/images/route.ts:after upload loop", message: "upload loop done", data: { urlsLength: urls.length }, hypothesisId: "H1" });
    // #endregion
    const inserted = await addProductImages(numId, urls);
    // #region agent log
    debugLog({ location: "app/images/route.ts:after addProductImages", message: "addProductImages done", data: { insertedLength: inserted.length }, hypothesisId: "H3" });
    // #endregion
    return NextResponse.json(inserted);
  } catch (err) {
    // #region agent log
    const errObj = err instanceof Error ? { name: err.name, message: err.message, code: (err as { code?: string }).code } : { err: String(err) };
    debugLog({ location: "app/images/route.ts:catch", message: "POST upload failed", data: errObj, hypothesisId: "H1-H5" });
    // #endregion
